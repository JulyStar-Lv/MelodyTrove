...
class RecentlyPlayedViewModel(
        ...
        viewModelScope.launch {
            ...
            try {
                ...
                _state.value = _state.value.copy(
                    isLoading = false,
                    error = e.message ?: "Failed to load recently played",
                )
            }
        }
    }
    ...
    private fun downloadTrack(track: RecentlyPlayedTrackItem) {
        val mediaId = track.mediaId ?: run {
            viewModelScope.launch {
                _events.send(RecentlyPlayedEvent.ShowMessage("This track cannot be downloaded yet."))
            }
            return
        }
        viewModelScope.launch {
            try {
                ...
                _events.send(RecentlyPlayedEvent.ShowMessage("Added to Downloads."))
            } catch (e: CancellationException) { throw e }
            catch (e: Exception) {
                _events.send(RecentlyPlayedEvent.ShowMessage(e.message?.takeIf { it.isNotBlank() } ?: "Failed to add download."))
            }
        }
    }
}
