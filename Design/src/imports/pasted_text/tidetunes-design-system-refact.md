Refactor and visually optimize the existing TideTunes Design System / app prototype in this Figma Make project so it matches the CURRENT production Compose Multiplatform repository. Do not create a separate concept and do not only explain. Edit the existing project files, keep all current routes working, and finish with a compiling preview.

Product direction
- Keep the existing TideTunes identity and the HyperOS / compose-miuix feel: calm, layered, rounded, music-first, dark-mode-first. Avoid generic dashboard styling and avoid direct iOS imitation.
...
- TideButton variants Primary, Secondary, Tertiary, Ghost, Danger; minimum height 40, 16x8 padding, full pill radius, with disabled states.
- TideCardSurface: 28 radius, 16 padding, 1px semantic outline, flat semantic surface by default.
- TideSearchBar: 48 minimum height, 16 radius, 16 horizontal padding, 16px search icon, correct empty/filled/disabled states.
...
- Update Foundation, Tokens, Components, Patterns and Compose pages so examples and Compose mappings use the exact specs above and real names from the repository (TideButton, TideCardSurface, TideSearchBar, TideTabs, TidePageHeader, TideSettingsGroup, TidePreferenceRow, TideIconButton, TidePlayerControlButton).
