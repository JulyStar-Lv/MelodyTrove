...
use tidetune_order_key::OrderKeyError;

#[derive(Debug, thiserror::Error, uniffi::Error)]
#[uniffi(flat_error)]
pub enum BError {
    #[error("remote storage error: {0:?}")]
    RemoteStorageError(#[from] tidetune_remote_storage::StorageBackendError),
    #[error("metadata error: {0}")]
    MetadataError(#[from] tidetune_metadata::MetadataError),
