...
use tidetune_remote_storage::{ByteRange, StorageBackend};
...
pub async fn start_playback_gateway(
    ...
    tidetune_runtime::tokio_runtime().spawn(async move {
...
mod tests {
    use super::*;
    use tidetune_remote_storage::LocalBackend;
    ...
    async fn streams_bounded_ranges_over_loopback() {
        let path = std::env::temp_dir().join(format!("tidetune-range-{}.flac", random_token()));
