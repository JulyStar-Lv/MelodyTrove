...
pub async fn start_playback_gateway(
    ...
    tidetunes_runtime::tokio_runtime().spawn(async move {
...
mod tests {
    use super::*;
    use tidetunes_remote_storage::LocalBackend;
    ...
    async fn streams_bounded_ranges_over_loopback() {
        let path = std::env::temp_dir().join(format!("tidetunes-range-{}.flac", random_token()));
