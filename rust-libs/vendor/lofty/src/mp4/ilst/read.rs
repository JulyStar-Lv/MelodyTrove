	...
	R: Read + Seek,
{
	...
	while let Ok(Some(atom)) = ilst_reader.next() {
		if let AtomIdent::Fourcc(ref fourcc) = atom.ident {
			match fourcc {
				b"free" | b"skip" => {
					skip_atom(ilst_reader, atom.extended, atom.len)?;
					continue;
				},
				b"covr" => {
					if parse_options.read_cover_art {
						handle_covr(ilst_reader, parsing_mode, &mut tag, &atom)?;
					} else {
						if parse_options.read_cover_art_presence {
							tag.atoms.push(Atom {
								ident: AtomIdent::Fourcc(*b"covr"),
								data: AtomDataStorage::Single(AtomData::Picture(Picture::EMPTY)),
							});
						}
						skip_atom(ilst_reader, atom.extended, atom.len)?;
					}
				...
				b"gnre" if parse_options.implicit_conversions => {
					...
					if let Some(atom_data) =
						parse_data_inner(ilst_reader, parsing_mode, &atom)?
					{
		...
		parse_data(ilst_reader, parsing_mode, &mut tag, atom)?;
	}
