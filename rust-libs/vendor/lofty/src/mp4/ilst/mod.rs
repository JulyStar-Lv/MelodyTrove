...
mod tests {
	...
	#[test_log::test]
	fn skips_cover_payload_while_retaining_presence() {
		...
		let mut bytes = Vec::with_capacity(payload_size + 8);
		...
		let mut reader = AtomReader::new(source, ParsingMode::BestAttempt).unwrap();
		let ilst = super::read::parse_ilst(
			&mut reader,
			ParseOptions::new()
