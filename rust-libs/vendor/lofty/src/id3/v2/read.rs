...
use super::items::AttachedPictureFrame;
...
where
	R: Read,
{
	parse_id3v2_with_skip(bytes, header, parse_options, |reader, size| {
		skip_by_reading(reader, size)
	})
}

pub(crate) fn parse_id3v2_seek<R>(
	bytes: &mut R,
	header: Id3v2Header,
	parse_options: ParseOptions,
) -> Result<Id3v2Tag>
...
{
	parse_id3v2_with_skip(bytes, header, parse_options, |reader, size| {
		let available = reader.limit();
		let skipped = u64::from(size).min(available);
		reader.get_mut().seek(SeekFrom::Current(skipped as i64))?;
		reader.set_limit(available - skipped);
		Ok(())
	})
}

fn parse_id3v2_with_skip<R, S>(
	bytes: &mut R,
	header: Id3v2Header,
	parse_options: ParseOptions,
	mut skip_cover_art: S,
) -> Result<Id3v2Tag>
where
	R: Read,
	S: FnMut(&mut Take<&mut R>, u32) -> Result<()>,
{
	log::debug!(
	...
	if header.flags.unsynchronisation {
		...
		ret = read_all_frames_into_tag(&mut tag_bytes, header, parse_options, &mut skip_cover_art)?;
	}
...
fn read_all_frames_into_tag<R, S>(
	reader: &mut R,
	header: Id3v2Header,
	parse_options: ParseOptions,
	skip_cover_art: &mut S,
) -> Result<Id3v2Tag>
where
	R: Read,
	S: FnMut(&mut R, u32) -> Result<()>,
{
	...
	loop {
		match ParsedFrame::read(reader, header.version, parse_options, skip_cover_art)? {
			...
			ParsedFrame::Artwork => {
				tag.insert(Frame::Picture(AttachedPictureFrame::new(
					TextEncoding::UTF8,
					Picture::unchecked(Vec::new()).build(),
				)));
			},
...
fn skip_by_reading(reader: &mut impl Read, size: u32) -> Result<()> {
