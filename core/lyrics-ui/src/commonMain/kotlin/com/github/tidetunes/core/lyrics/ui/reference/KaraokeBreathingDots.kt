...
data class KaraokeBreathingDotsDefaults(
    ...
    val breatheScaleMidpoint: Float = 0.9f,
    val breatheScaleAmplitude: Float = 0.1f,
)
...
internal fun resolveBreathingDotsBreatheScale(
    ...
    scaleMidpoint: Float = 0.9f,
    scaleAmplitude: Float = 0.1f
): Float {
    ...
    return scaleMidpoint - scaleAmplitude * cos(angle)
}
            ...
            when {
                ...
                currentTime < timeline.dipStart -> {
                    ...
                    scale = resolveBreathingDotsBreatheScale(
                        ...
                        scaleMidpoint = defaults.breatheScaleMidpoint,
                        scaleAmplitude = defaults.breatheScaleAmplitude
                    )
                }
                // Stage 3: Pre-exit
                currentTime < timeline.stillStart -> {
                    ...
                    scale = defaults.minScale + (1f - defaults.minScale) * cos(progress * 2 * PI).toFloat()
                }
                // Stage 4: Still
