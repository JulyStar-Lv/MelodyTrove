...
TideTunes keeps diagnostics local and starts a minimal Rust runtime before Koin, Room, the
...
```bash
TIDETUNES_DEVELOPER_MODE=true ./gradlew :desktopApp:run
```
...
NeriPlayer is GPLv3; TideTunes did not copy its Kotlin, C++, or other implementation code.
