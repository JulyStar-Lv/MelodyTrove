...
TideTunes supports SMB2 and SMB3 shares as indexed music sources. SMB1 is not
...
dynamic library. TideTunes uses bounded connection and reader pools around
...
Encryption protects SMB traffic between TideTunes and the server. The
...
source-scoped indexed search. TideTunes does not issue live full-text queries
to the SMB server.
...
short-lived localhost gateway after TideTunes has been terminated. The first
...
```bash
export TIDETUNES_SMB_TEST_AUTH_URL='smb://127.0.0.1/authenticated'
export TIDETUNES_SMB_TEST_GUEST_URL='smb://127.0.0.1/guest'
export TIDETUNES_SMB_TEST_USERNAME='<ephemeral-user>'
export TIDETUNES_SMB_TEST_PASSWORD='<ephemeral-password>'
export TIDETUNES_SMB_TEST_FIXTURE_DIR='<writable-fixture-directory>'
cargo test --manifest-path rust-libs/Cargo.toml \
  --package tidetunes-storage-backend --test smb_integration \
  -- --ignored --nocapture
```
