...
object SafeModeRecoveryStore {
    ...
    private val planPath: Path
        get() = "${getAppDocumentDir()}/diagnostics/state/recovery-options.json".toPath()
    ...
    fun backupSettings(): String = backupFiles(
        prefix = "settings",
        paths = listOf("${getAppDocumentDir()}/tidetunes.preferences_pb".toPath()),
    )
    ...
    private fun backupFiles(prefix: String, paths: List<Path>): String {
        ...
        val destination = (
            "${getAppDocumentDir()}/backups/recovery-$prefix-${currentTimeMillis()}"
            ).toPath()
